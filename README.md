...
| `add_ingredients` | 식재료 일괄 추가 — 장본 품목 전체를 한 번에 등록 (유통기한 `YYYY-MM-DD` 또는 `YYYYMMDD` 선택 입력) |
...
| 추가 | "계란 10개 샀어. 유통기한은 7월 20일이야" | `add_ingredients` → 추가 결과 + 갱신된 표 |
...
사용자: "냉장고에 있는 걸로 뭐 만들어 먹을까?"
  → AI: list_ingredients 호출 (표는 표시하지 않고 참고만)
  → AI: 임박 재료(🟡) 우선으로 만들 수 있는 음식 3가지 제시
        "어떤 음식의 레시피가 궁금하신가요?"
사용자: "두부조림"
  → AI: 레시피(재료·수량·조리법) 제시
...
사용자: "장 보고 왔어"
  → [2단계] AI: "안내드린 재료를 그대로 사오셨나요?" 확인
        → 실제 구매 품목 전체를 add_ingredients로 한 번에 등록
